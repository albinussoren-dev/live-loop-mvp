...
class YouTubeService:
    ...
    def exchange_code(self, code):
        f = self._flow(); f.fetch_token(code=code); return f.credentials

    def userinfo(self, token: str):
        req = Request("https://openidconnect.googleapis.com/v1/userinfo", headers={"Authorization": f"Bearer {token}"})
        with urlopen(req, timeout=15) as response:
            return json.loads(response.read().decode("utf-8"))

    def credentials(self, token, refresh_token=None, expiry=None):
        c = Credentials(token=token, refresh_token=refresh_token,
                        token_uri="https://oauth2.googleapis.com/token",
    ...
    def api(self, token, refresh_token=None):
    ...
    def videos(self, token, refresh_token=None, limit=50):
        ...
        while len(out) < limit:
            r = yt.playlistItems().list(part="snippet,contentDetails", playlistId=pid,
                                        maxResults=min(50, limit-len(out)), pageToken=page).execute()
            ids = [x["contentDetails"]["videoId"] for x in r.get("items", [])]
            if ids:
                out.extend(yt.videos().list(part="snippet,contentDetails,status", id=",".join(ids)).execute().get("items", []))
            page = r.get("nextPageToken")
            if not page: break
        return out[:limit]

    def create_broadcast(self, token, title, privacy="unlisted", description="", scheduled_start=None):
        start = scheduled_start or (datetime.now(timezone.utc) + timedelta(minutes=1)).isoformat().replace("+00:00", "Z")
        return self.api(token).liveBroadcasts().insert(
            part="snippet,status,contentDetails",
            body={"snippet":{"title":title,"description":description,"scheduledStartTime":start},
    ...
    def create_stream(self, token, title, resolution="1080p", fps="30fps"):
        return self.api(token).liveStreams().insert(
            part="snippet,cdn,contentDetails,status",
            body={"snippet":{"title":title},"cdn":{"ingestionType":"rtmp","resolution":resolution,"frameRate":fps}}
        ).execute()
    ...
    def broadcast(self, token, broadcast_id):
        r = self.api(token).liveBroadcasts().list(part="id,snippet,status,contentDetails", id=broadcast_id).execute()
        return (r.get("items") or [None])[0]
